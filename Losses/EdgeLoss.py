...
class EdgeLoss(LossInterface):
    ...
    def parse_settings(self, args):
        ...
        if isinstance(args.edge_color, str):
    ...
    def get_loss(self, cur_cutouts, out, args, globals=None, lossGlobals=None):
        ...
        if self.resized_mask is None and self.mask is not None:
            self.resized_mask = TF.to_tensor(
                self.mask).to(
                self.device).unsqueeze(0)
            self.resized_mask = TF.resize(
                self.resized_mask, out.size()[
                    2:4], TF.InterpolationMode.BICUBIC)
        ...
        if not self.mask:
            ...
            rloss = mseloss(out[:, :, :, rmax - right:],
                            zers[:, :, :, rmax - right:])
            uloss = mseloss(out[:, :, :upper, left:rmax - right],
                            zers[:, :, :upper, left:rmax - right])
            dloss = mseloss(out[:, :, lmax -
                                lower:, left:rmax -
                                right], zers[:, :, lmax -
                                             lower:, left:rmax -
                                             right])
        ...
        if args.global_color_weight:
            gloss = mseloss(out[:, :, :, :], zers[:, :, :, :]
                            ) * args.global_color_weight
            cur_loss += gloss
