...
class CLIP_Base():
    ...
    def __init__(self, model, preprocess, device):
        ...
        self.preprocess_transform = transforms.Compose(
            [
                transforms.Resize(
                    self.input_resolution), transforms.CenterCrop(
                    self.input_resolution), transforms.Normalize(
    ...
    def encode_texts(self, texts):
        text_embeddings = torch.stack([self.model.encode_text(
            clip.tokenize(text).to(self.device)).detach().clone() for text in texts])
...
class SLIP_Base():
    def __init__(self, model_name, device):
        ...
        print("Loaded perceptor %s: %.2fM params" %
              (model_name, (n_params / 1000000)))
...
def get_clip_perceptor(clip_model_name, device):
    if clip_model_name in clip.available_models():
        perceptor, preprocess = clip.load(
            clip_model_name, download_root="models")
