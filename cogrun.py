...
class BasePixrayPredictor(BasePredictor):
    ...
    **kwargs) -> Iterator[Path]:
        ...
        with open(settings_file, 'r') as stream:
          try:
              base_settings = yaml.safe_load(stream) or {}
...
class PixrayVqgan(BasePixrayPredictor):
    def predict(self,
        ...
        yield from super().predict(settings="pixray_vqgan")
...
class PixrayPixel(BasePixrayPredictor):
    def predict(self,
        ...
        yield from super().predict(settings="pixray_pixel")
...
class Text2Pixel(BasePixrayPredictor):
    def predict(self,
        ...
        yield from super().predict(settings="text2pixel")
